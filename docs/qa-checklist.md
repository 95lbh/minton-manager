...
- [ ] 자동 배정 우선순위(균형/다양성/실력) 선택 → 추천 결과가 우선순위에 맞게 달라짐(다양성=파트너/상대 반복 회피, 실력=팀 실력차 최소)
...
- [ ] 소유권 이임: 소유자가 공동 관리자에게 이임(확인창) → 새 소유자로 변경, 이전 소유자는 일반 운영자(삭제 권한 상실) · 자기 자신/임시 클럽 차단 · 비소유자에겐 버튼 미노출
- [ ] 전체화면 토글
- [ ] 실시간 동기화: 두 기기(또는 두 탭)에서 한쪽이 출석 체크인/코트 배정/게임 종료 → 다른 쪽 화면이 자동 갱신(0017 퍼블리케이션 적용 필요). 타 클럽엔 미전파(RLS)
